...
# Job name
#SBATCH --job-name=FunannotateAnnotate-BBQ
...
# ----------------------------------------------- #
# 7. Launch InterProScan for proteins annotation  #
# ----------------------------------------------- #

module load interproscan/5.59-91.0

        funannotate iprscan -i $Y"Bloom" -m local --no-progress --iprscan_path interproscan.sh

# ----------------------------------- #
# 8. Annotate the predicted genes :   #
# ----------------------------------- #

        cp $Y"Bloom"/annotate_misc/iprscan.xml $Y"Bloom"
        grep -v 'go-xref db="GO"' $Y"Bloom"/iprscan.xml > $Y"Bloom"/annotate_misc/iprscan.xml

        funannotate annotate -i $Y"Bloom" --cpus 16 --no-progress
