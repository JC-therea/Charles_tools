...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print("Usage: python mergeTogaGMAP.py original.gtf funannotate.gtf output.gtf")
        sys.exit(1)
    ...
    for _, transcript_b in transcripts_b.iterrows():
        ...
        if parent_b in eqGenes:
            transcriptNumber = id_b.split(".mrna")[1]
            new_id_b = eqGenes[parent_b] + ".mrna" + transcriptNumber
    ...
    for _, other_b in other_bs.iterrows():
        ...
        if parent_b in eqGenes and other_b['type'] == 'five_prime_UTR':
            assignedNumber = id_b.split(".mrna")[1]
            new_id_b = eqGenes[parent_b] + ".mrna" + assignedNumber
            eqGenes[id_b] = new_id_b
