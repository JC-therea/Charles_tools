...
# Keep only uORF, ouORF, dORF and odORF
SparORF_table = pd.read_csv(SparORF_table_path, names=genePredHeader, sep="\t")
SparORF_table["transcript"] = SparORF_table.orfID.replace(":.*", "", regex = True)
SparORF_table["gene"] = SparORF_table.transcript.replace("-T[1-9]", "", regex = True)
SparORF_table["transcriptInit"] = SparORF_table.orfID.str.extract(patternTranscriptInit).astype(int)
SparORF_table["transcriptEnd"] = SparORF_table.orfID.str.extract(patternTranscriptEnd).astype(int)
SparORF_table["transcriptType"] = SparORF_table.orfID.str.extract(patternTranscriptType)
SparORF_table = SparORF_table[SparORF_table.transcriptType.isin(orfTypes)]

SbayORF_table = pd.read_csv(SbayORF_table_path, names=genePredHeader, sep="\t")
SbayORF_table["transcript"] = SbayORF_table.orfID.replace(":.*", "", regex = True)
SbayORF_table["gene"] = SbayORF_table.transcript.replace("-T[1-9]", "", regex = True)
SbayORF_table["transcriptInit"] = SbayORF_table.orfID.str.extract(patternTranscriptInit).astype(int)
SbayORF_table["transcriptEnd"] = SbayORF_table.orfID.str.extract(patternTranscriptEnd).astype(int)
SbayORF_table["transcriptType"] = SbayORF_table.orfID.str.extract(patternTranscriptType)
SbayORF_table = SbayORF_table[SbayORF_table.transcriptType.isin(orfTypes)]
